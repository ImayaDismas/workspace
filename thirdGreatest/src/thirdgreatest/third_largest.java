...
//but the output should be world because it appeared as the last 5 letter word in the array. If strArr 
//was ["hello", "world", "after", "all"] the output should be after because the first 
...
public class third_largest{
	public static String ThirdGreatest(String[] strArr){
		int[] size =new int[strArr.length];
			...
			String thirdString = "";
			for(int i=0; i<strArr.length; i++){
				if(strArr[i].length() == size[size.length - 3]){
					 thirdString = strArr[i];
				}
			}
			return thirdString;
		}
	public static void main(String[] args){
		String[] Array = {"hello", "world", "kdssa","kdssi", "before","kdssg", "all"};
		System.out.println(ThirdGreatest(Array));
	}
}
